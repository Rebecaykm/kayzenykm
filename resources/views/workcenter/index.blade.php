<x-app-layout title="Centro de Trabajo">
    <div class="container grid px-6 mx-auto">
        ...
        <!-- <div class="flex justify-end mb-4">
            <a href="{{ route('workcenter.create') }}" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                {{ __('Nuevo Centro de Trabajo') }}
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <!-- <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    ...
                                    <form action="{{ route('workcenter.destroy', $workcenter->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                <span class="flex items-center col-span-3">
                    {{ __('Mostrando') }} {{ $workcenters->firstItem() }} - {{ $workcenters->lastItem() }} {{ __('de') }} {{ $workcenters->total() }}
                </span>
            ...
            <div class="px-4 py-3 rounded-md text-sm text-center font-semibold text-gray-700 uppercase bg-white sm:grid-cols-9 dark:text-gray-500 dark:bg-gray-800">
                {{ __('No Se Han Encontrado Centros de Trabajo') }}
            </div>
            @endif
        </div>
    </div>
