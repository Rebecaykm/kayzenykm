<x-app-layout title="Tipo de Paro">
    <div class="container grid px-6 mx-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
                        ...
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    ...
                                    <form action="{{ route('unemployment-type.destroy', $unemploymentType->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
            ...
            @if ($unemploymentTypes->count())
            <div class="grid px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
                <span class="flex items-center col-span-3">
                    {{ __('Mostrando') }} {{ $unemploymentTypes->firstItem() }} - {{ $unemploymentTypes->lastItem() }} {{ __('de') }} {{ $unemploymentTypes->total() }}
                </span>
                <span class="col-span-2"></span>
                <!-- Pagination -->
                <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                    <nav aria-label="Table navigation">
                        {{ $unemploymentTypes->withQueryString()->links() }}
                    </nav>
                </span>
            </div>
            @else
            <div class="px-4 py-3 rounded-md text-sm text-center font-semibold text-gray-700 uppercase bg-white sm:grid-cols-9 dark:text-gray-500 dark:bg-gray-800">
                {{ __('No Se Han Encontrado Tipos de Paro') }}
            </div>
            @endif
        </div>
    </div>
