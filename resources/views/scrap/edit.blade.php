<x-app-layout title="Tipo de Scrap">
    <div class="container grid px-6 mx-auto">
        ...
        <form action="{{ route('scrap.update', $scrap->id) }}" method="post">
            @csrf
            @method('PUT')
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">{{ __('Código') }}</span>
                    <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" value="{{ $scrap->code }}" type="text" name="code" autocomplete="off" />
                </label>
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">{{ __('Nombre') }}</span>
                    <input class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" value="{{ $scrap->name }}" type="text" name="name" autocomplete="off" />
                </label>
                <label class="block mt-4 text-sm">
                    ...
                    <select name="type_scrap_id" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                        ...
                        <option {{ $scrap->type_scrap_id == $typeScrap->id ? "selected" : "" }} value="{{ $typeScrap->id }}">{{ $typeScrap->name }}</option>
                        @endforeach
                    </select>
                </label>
